...
if (isset($_POST['start']) && isset($_POST['end'])) {
    ...
    if (!$db_selected) {
        ...
        $query_del=  sprintf("DELETE FROM `temp_events` WHERE 1");
        ...
        if($active_flag){
            ...
            for($i=0;$i<count($lat_array);$i++)
            {
                ...
                if($b->contains($lt))
                {
                    ...
                    */
                     $queryT = sprintf("INSERT INTO temp_events " .
                     " (id,lat,lng,name,description,type,starttime,endtime,regid) " .
                     " VALUES (NULL, '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s');",
                     mysql_real_escape_string($row['lat']),
                     mysql_real_escape_string($row['lng']),
                     mysql_real_escape_string($row['name']),
                     mysql_real_escape_string($row['description']),
                     mysql_real_escape_string($row['type']),
                     mysql_real_escape_string($startDateTime),
                     mysql_real_escape_string($endDateTime),
                     mysql_real_escape_string($reg_id));
                    $test = mysql_query($queryT);
                    
                    $post_message[]=$row['id'].",".$row['lat'].",".$row['lng'].",".$row['name'].",".$row['description'].",".$row['type'].",".$startDateTime;
                    break;   
                }
